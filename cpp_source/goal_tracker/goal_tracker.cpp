...
{
  setModuleDescription("C++ implementation of locateGoalzor.py");
  ...
  functionName("track", getName(), "finds a goal");
  addParam("im_string", "String representing a Python image");
  addParam("size_x", "Horizontal size of image");
  addParam("size_y", "Vertical size of image");
  addParam("yawHead", "Yaw parameter of the Nao head");
  addParam("color", "Colour of the goal");
  setReturn("return", "String containing the found location");
  BIND_METHOD(goal_tracker::track);
}
...
IplImage* goal_tracker::markGoalCV(IplImage *im, string color)
{
    /**
...
double goal_tracker::calcXangle(int xcoord)
{
    /**
     * As made by Michael Cabot
     * Ported to C++ by Maarten de Jonge
...
tuple goal_tracker::run(IplImage* image, double yawHead, string color)
{
    /**
     * By Author
     * Ported to C++ by Maarten de Jonge
        ...
        const double& yawHead, const string& color)
{
    ...
    cvSetData(img, const_cast<char*>(im_string.c_str()), size_x*3);

    // find the goal
    tuple goal = run(img, yawHead, color);

    // deallocating the image
    cvReleaseImageHeader(&img);

    // FIXME
    // hacky method of returning the output as a string
    stringstream sstr;
    sstr << goal.first;
    sstr << " ";
    sstr << goal.second;
